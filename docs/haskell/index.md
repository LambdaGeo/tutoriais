...
!!! note
    **Nota desta edição revisada (v3).** O texto original tem mais de quinze anos, e o ecossistema Haskell mudou bastante. Nesta revisão: (1) instalação e criação de projetos atualizadas para o fluxo moderno com **GHCup** e **Cabal** (sem Stack — o `.cabal` é escrito diretamente, sem a camada `package.yaml`/hpack); (2) código ajustado para **GHC 9.x** — em particular, desde o GHC 8.4 o operador `<>` faz parte do Prelude, o que exige um ajuste que o livro não previa; (3) as saídas do QuickCheck 2.14 atualizadas, a seção do `quickCheckAll` reescrita (com uma correção no código de saída da suíte) e a seção do **HPC** refeita para as ferramentas atuais; (4) **todo o código e todas as saídas de terminal foram executados e conferidos** com GHC 9.4/9.10, Cabal e QuickCheck 2.14.3 — incluindo os números exatos do relatório de cobertura do HPC (Capítulo 9), reproduzidos byte a byte; e (5) o exercício 3 do Capítulo 9 ganhou um aviso concreto sobre uma armadilha real de geradores recursivos com listas (`sized` sem `resize`), descoberta ao implementar a extensão do gerador para o tipo `JValue`.
